...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name} (${upper}) 시세 — Economic AI Dashboard`,
    description: `${name} 실시간 코인 시세, 차트, AI 투자 영향도 분석을 한눈에 확인하세요.`,
    openGraph: {
      title: `${name} 시세`,
      description: `${name} (${upper}) 실시간 코인 시세 및 AI 분석`,
      type: "website"
    }
  };
}
...
export default async function CryptoDetailPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "Dataset",
    name: `${name} 코인 시세 데이터`,
    description: `${name} (${upper}) 실시간 시세 및 AI 투자 영향도 분석`,
